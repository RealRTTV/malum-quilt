...
public class SoulwoodTreeFeature extends Feature<DefaultFeatureConfig> { // todo make abstract tree feature regardless of tree
    ...
    public static void downwardsTrunk(StructureWorldAccess world, MalumFiller filler, BlockPos pos) {
        ...
        do {
            ...
            if (canPlace(world, trunkPos)) {
                filler.entries.add(new BlockStateEntry(SOULWOOD_LOG.getDefaultState(), trunkPos));
            } else {
                break;
            }
    ...
    public static boolean canPlace(StructureWorldAccess level, BlockPos pos) {
        ...
        return state.isOf(SOULWOOD_SAPLING) || level.isAir(pos) || state.getMaterial().isReplaceable();
    }
    ...
    public boolean place(FeatureContext<DefaultFeatureConfig> ctx) {
        ...
        if (world.isAir(pos.down()) || !SOULWOOD_SAPLING.getDefaultState().canPlaceAt(world, pos)) {
            return false;
        }
        BlockState defaultLog = SOULWOOD_LOG.getDefaultState();
        ...
        for (Integer index : sapBlockIndexes) {
            ...
            BlockState newState = BlockHelper.getBlockStateWithExistingProperties(oldEntry.state(), EXPOSED_SOULWOOD_LOG.getDefaultState());
