...
public class ParticleBuilders {
    ...
    public static class WorldParticleBuilder {
        ...
        public WorldParticleBuilder setForcedMotion(Vec3f startingMotion, Vec3f endingMotion) {
        ...
        public WorldParticleBuilder spawnCircle(World world, double x, double y, double z, double distance, double currentCount, double totalCount) {
        ...
        public WorldParticleBuilder spawn(World world, double x, double y, double z) {
        ...
        public WorldParticleBuilder evenlySpawnAtEdges(World world, BlockPos pos) {
            for (Direction direction : Direction.values()) {
                ...
                double d0 = 0.5625D;
                this.dx = direction$axis == Direction.Axis.X ? 0.5D + d0 * (double) direction.getOffsetX() : (double) random.nextFloat();
                this.dy = direction$axis == Direction.Axis.Y ? 0.5D + d0 * (double) direction.getOffsetY() : (double) random.nextFloat();
                this.dz = direction$axis == Direction.Axis.Z ? 0.5D + d0 * (double) direction.getOffsetZ() : (double) random.nextFloat();
        ...
        public WorldParticleBuilder evenlySpawnAtEdges(World world, BlockPos pos, Direction... directions) {
        ...
        public WorldParticleBuilder spawnAtEdges(World world, BlockPos pos) {
            Direction direction = Direction.values()[world.random.nextInt(Direction.values().length)];
            double yaw = random.nextFloat() * Math.PI * 2, pitch = random.nextFloat() * Math.PI - Math.PI / 2, xSpeed = random.nextFloat() * maxXSpeed, ySpeed = random.nextFloat() * maxYSpeed, zSpeed = random.nextFloat() * maxZSpeed;
            this.vx += Math.sin(yaw) * Math.cos(pitch) * xSpeed;
            this.vy += Math.sin(pitch) * ySpeed;
            this.vz += Math.cos(yaw) * Math.cos(pitch) * zSpeed;
        ...
        public WorldParticleBuilder repeat(World world, double x, double y, double z, int n) {
            for (int i = 0; i < n; i++) spawn(world, x, y, z);
            return this;
        }

        public WorldParticleBuilder repeatEdges(World world, BlockPos pos, int n) {
            for (int i = 0; i < n; i++) spawnAtEdges(world, pos);
            return this;
        }

        public WorldParticleBuilder evenlyRepeatEdges(World world, BlockPos pos, int n) {
            for (int i = 0; i < n; i++) evenlySpawnAtEdges(world, pos);
            return this;
        }

        public WorldParticleBuilder evenlyRepeatEdges(World world, BlockPos pos, int n, Direction... directions) {
            for (int i = 0; i < n; i++) evenlySpawnAtEdges(world, pos, directions);
            return this;
        }

        public WorldParticleBuilder repeatCircle(World world, double x, double y, double z, double distance, int times) {
