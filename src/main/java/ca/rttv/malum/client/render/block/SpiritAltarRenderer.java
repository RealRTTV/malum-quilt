...
public class SpiritAltarRenderer implements BlockEntityRenderer<SpiritAltarBlockEntity> {
    ...
    public void render(SpiritAltarBlockEntity blockEntity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        for (int i = 0; i < inventory.size(); i++) {
            ...
            if (!item.isEmpty()) {
                matrices.push();
                Vec3f offset = new Vec3f(SpiritAltarBlockEntity.spiritOffset(blockEntity, i));
                matrices.translate(offset.getX(), offset.getY(), offset.getZ());
                matrices.multiply(Vec3f.POSITIVE_Y.getDegreesQuaternion((level.getTime() ) * 3 + tickDelta));
        ...
        ItemStack stack = blockEntity.heldItem.get(0);
        if (!stack.isEmpty()) {
            matrices.push();
