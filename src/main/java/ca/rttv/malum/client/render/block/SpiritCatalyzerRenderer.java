...
public class SpiritCatalyzerRenderer implements BlockEntityRenderer<SpiritCatalyzerBlockEntity> {
    @Override
    public void render(SpiritCatalyzerBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        if (!stack.isEmpty() && world != null) {
            ...
            if (stack.getItem() instanceof SpiritItem) {
                double y = Math.sin((world.getTime() % 710 + tickDelta) / 20f) * 0.05f;
